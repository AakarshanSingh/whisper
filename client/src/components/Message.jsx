  ...
  return (
    ...
    >
      <div className={`chat ${chatClassName} max-w-xs lg:max-w-md xl:max-w-lg`}>
        <div
          className={`chat-bubble ${bubbleClass} shadow-sm rounded-xl px-4 py-3 relative transition-all duration-200 ${
            fromMe ? 'rounded-br-sm ml-auto' : 'rounded-bl-sm mr-auto'
          } ${isHovered ? 'shadow-md' : ''}`}
        >
          {/* Message content */}
          {!message.image && (
        ...
        <div
          className={`chat-footer mt-1 flex items-center gap-2 transition-opacity duration-200 ${
